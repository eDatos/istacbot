...
ruleNextTime :: Rule
ruleNextTime = Rule
  { name = "el proximo <time>"
  , pattern =
    [ regex "(el)?"
    , regex "pr(ó|o)ximo|siguiente"
...
ruleTT = Rule
  ...
  , prod = \case
      (Token RegexMatch (GroupMatch (t:qq:_)):_) -> do
        q <- parseInt qq
...
ruleCycleLastOrdinalOfTime = Rule
  { name = "<ordinal> ultimo <cycle> de <time>"
  , pattern =
    [ dimension Ordinal
    , regex "(u|ú)ltimo"
    , dimension TimeGrain
...
ruleElProximoqueVieneCycle :: Rule
ruleElProximoqueVieneCycle = Rule
  { name = "el (proximo|que viene) <cycle>"
  , pattern =
    [ regex "(el|los|la|las)?"
    , regex "(pr(ó|o)xim(o|a)s?|que vienen?|siguientes?)"
    , dimension TimeGrain
...
ruleProximoqueVieneCycle :: Rule
ruleProximoqueVieneCycle = Rule
  { name = "(proximo|que viene) <cycle>"
  , pattern =
    [ regex "(pr(ó|o)xim(o|a)s?|que vienen?|siguientes?)"
    , dimension TimeGrain
    ]
...
ruleCycleQuarterOrdinalYear :: Rule
ruleCycleQuarterOrdinalYear = Rule
  { name = "quarter <ordinal> <year>"
  , pattern =
    [ regex "trimestre"
    , dimension Ordinal
    , dimension Time
...
rules =
  ...
  , ruleProximoCycle
  , ruleElProximoqueVieneCycle
  , ruleProximoqueVieneCycle
  , ruleCycleQuarterOrdinalYear
 ]
  ++ ruleDaysOfWeek
  ++ ruleMonths
  ++ ruleSeasons
